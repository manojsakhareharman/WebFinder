...
feature "Translators translate a venue" do
  ...
  scenario "navigates to translation page for assigned locale" do
    ...
    expect(page).to have_link("Venues", href: cms_available_locale_venues_path(@locale))
  end

  scenario "translates a record" do
    venue = FactoryGirl.create(:venue) # which means it needs translation
    ...
    expect(venue.description).to eq("translated description for locale")

    I18n.locale = I18n.default_locale
    expect(venue.description).not_to eq("translated description for locale")

    expect(current_path).to eq(cms_available_locale_venues_path(@locale))
  end
end
